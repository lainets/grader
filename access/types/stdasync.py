...
if not hasattr(runner_module, "run"):
    raise AttributeError(f"{settings.RUNNER_MODULE} does not have a run function")
runner_func = runner_module.run
if not callable(runner_func):
    raise AttributeError(f"run attribute in {settings.RUNNER_MODULE} is not callable")
...
def _acceptSubmission(request, course, exercise, post_url, sdir: SubmissionDir):
    ...
    if not isinstance(ro_mounts, dict):
        raise ConfigError("'container'->'mounts' must not be specified or be a <path on course>-<mount path> dictionary")
    for k,v in ro_mounts.items():
        if not os.path.isabs(v):
            raise ConfigError("Mount path must be absolute")
        ...
        if os.path.isabs(k) or os.path.normpath(v).startswith(".."):
            raise ConfigError("Mounted path on course must be a relative subpath")
